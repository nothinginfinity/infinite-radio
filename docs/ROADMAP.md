...
### Step 1b — revision guard + durable note addressing — ACCEPTED
- design review from chatgpt:infinite-radio (msg:6c8119a9-a83e-499e-80a6-58c9e16a51fc) identified two prerequisites before any tool/MCP schema is exposed on top of Step 1; both are implemented and live-verified here
- `editor-state.js` sessions now carry a monotonically increasing `revision`, bumped by `dispatchEdit`/`undoEdit`/`redoEdit`/`resetDraft` and never by reads -- the reducer itself is the source of truth for the counter, not something bolted on only in the route layer
- every mutating draft route (`/draft/edit`, `/draft/undo`, `/draft/redo`, `/draft/reset`) requires `expectedRevision` in the request body and fails closed with `draft_revision_conflict` on mismatch, `draft_expected_revision_required` if omitted, leaving the draft untouched either way -- this is the concurrency guard a human browser, an internal co-producer LLM, and an external MCP agent all need before they can safely share one draft
- `EditNote` can now be addressed by an opaque `note_ref` (from the new `GET /draft/notes` read route) instead of a bare `trackId`/`eventIndex` pair, so future structural edits (and add/delete) can reorder or shift indices without silently mutating the wrong note; a `note_ref` binds the coordinates to the exact revision and a content fingerprint, and is rejected as `draft_note_ref_stale` or `draft_note_ref_invalid` rather than trusted blindly -- addressing indirection, not a security boundary, since the underlying `EditCommand` vocabulary and validator are unchanged
- commit `54aa2f1ec95d02da225ed100c943e19ac568ce8c` (reducer revision), `cf8a60952b7813ca121b529e20fd0ced4da39f2a`/`561c7d67c2f69e46292568b2cd0265e110339665` (routes + tests); CI `33557912600` and deploy + strict live + real Chromium/WebAudio `33557912609` both succeeded; the full revision-conflict and note_ref resolve/stale/forged-ref behavior was independently curled against the live production Worker
- still deliberately deferred: the browser editor itself still runs its own local in-memory session (unchanged from Step 1) and has not been switched to call these authoritative server routes -- V0.6 acceptance should not claim human-agent convergence until that switch happens, per the same design review

### Step 1c — serialized draft mutation boundary — ACCEPTED
- design review from chatgpt:infinite-radio (msg:ddc1d7c2-5b57-4c31-9be9-831dfb8e5ba1) found that the `expectedRevision` guard alone did not close the race: Durable Objects are single-threaded, but async awaits inside one request still let another request's handler interleave, so two concurrent callers could each load revision N, both pass the check before either persisted, and one edit could be silently lost
- `runDraftMutation` mirrors the existing `runCompositionMutation` promise-tail pattern and now wraps the entire critical section -- fresh session load, `expectedRevision` check, `note_ref` resolution, reducer dispatch, and persist -- as one ordered unit for every mutating draft route (`/draft/start`, `/draft/edit`, `/draft/undo`, `/draft/redo`, `/draft/reset`); a second call's closure only begins after the first has fully persisted, regardless of internal await count
- commit `6b7707ee173c08843013f0fbb027857398757c46` (fix), `f08079a6278f853bcef45188fd49dd64ccdbb32a` (concurrent-request regression test); CI `33576159039` and deploy + strict live + real Chromium/WebAudio `33576159029` both succeeded
- live-verified with genuinely concurrent shell-backgrounded curl requests (not just single-threaded-DO sequencing) against the deployed Worker: two simultaneous `/draft/edit` calls at `expectedRevision:0` produced exactly one 200 (revision 1) and one 400 `draft_revision_conflict`, with the final persisted draft matching the winner exactly -- no lost update, no double-advance

### Step 2 — tool/function-call schema over EDIT_COMMANDS — NEXT
- define the one-to-one tool schema (`music.get_score`, `music.get_section`, `music.set_tempo`, `music.edit_note`, `music.transpose`, `music.duplicate_section`, `music.undo`, `music.redo`, `music.preview`, etc.) that calls the Step 1/1b draft-session routes, including `expectedRevision` and `note_ref` in every relevant tool signature
- define tool descriptors and a single dispatcher once (e.g. a `music-tools` module) so internal function tools and the external MCP transport execute through the exact same schema/path -- MCP is a transport, not a second tool contract
- external MCP authorization must be stronger than the caller-supplied `x-creator-id` header used internally: reuse channel-ownership semantics underneath, but require a real authenticated/capability session for external agents, scoped to channel + allowed tool classes; the internal co-producer can use trusted in-process actor context
...
- switch (or explicitly synchronize) the browser editor onto the authoritative server draft session once the revision contract is proven, closing the human/agent-convergence gap noted above
