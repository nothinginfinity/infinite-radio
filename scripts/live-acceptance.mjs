...
assert.match(player.data.raw, /id=\"edit-open\"/);
assert.match(player.data.raw, /id=\"editor-sheet\"/);
assert.match(player.data.raw, /data-editor-preview=\"original\"/);
assert.match(player.data.raw, /data-editor-preview=\"draft\"/);
assert.match(player.data.raw, /data-editor-macro=\"brighter\"/);
assert.match(player.data.raw, /data-editor-macro=\"dry\"/);
assert.match(player.data.raw, /id=\"editor-arrange-map\"/);
assert.match(player.data.raw, /id=\"editor-section-energy\"/);
assert.match(player.data.raw, /id=\"editor-section-lift\"/);
assert.match(player.data.raw, /id=\"editor-section-drop\"/);
assert.match(player.data.raw, /id=\"editor-section-left\"/);
assert.match(player.data.raw, /id=\"editor-section-duplicate\"/);
assert.match(player.data.raw, /id=\"editor-section-right\"/);
...
assert.match(player.data.raw, /command\.type===\"section_duplicate\"/);
assert.match(player.data.raw, /command\.type===\"section_move\"/);
assert.match(player.data.raw, /command\.type===\"section_resize\"/);
assert.match(player.data.raw, /function editorLinearSections\(score\)/);
...
assert.match(player.data.raw, /Back to live discards the local preview/);
assert.match(player.data.raw, /const CROSSFADE_SECONDS=4/);
assert.match(player.data.raw, /adoptContext\(sharedContext\)/);
assert.match(player.data.raw, /rampMasterTo\(value,seconds=0\)/);
assert.match(player.data.raw, /startDualDeckCrossfade\(session\)/);
assert.match(player.data.raw, /crossfadeInFlight/);
assert.match(player.data.raw, /incoming\.adoptContext\(outgoing\.ctx\)/);
assert.match(player.data.raw, /outgoing\.rampMasterTo\(0,CROSSFADE_SECONDS\)/);
assert.match(player.data.raw, /incoming\.rampMasterTo\(\.5,CROSSFADE_SECONDS\)/);
assert.match(player.data.raw, /this\.master\.context!==this\.ctx/);
...
assert.match(player.data.raw, /async function resyncLiveAfterPlay\(session\)/);
const playStart = player.data.raw.indexOf("async function play(){");
const unlockIndex = player.data.raw.indexOf("await renderer.unlockFromGesture()", playStart);
const firstRendererPlayIndex = player.data.raw.indexOf("await renderer.play()", playStart);
const liveResyncIndex = player.data.raw.indexOf("void resyncLiveAfterPlay(stationState.session)", playStart);
...
assert.match(player.data.raw, /resumeIfPlaying&&wasPlaying&&!payload\.playback\.ended/);
...
assert.match(player.data.raw, /Crossfade unavailable · end transition fallback remains active/);
assert.match(player.data.raw, /stationState\.queuedCount<1&&!viewState\.replaying/);
assert.match(player.data.raw, /stationState\.autoAdvance&&!viewState\.replaying&&!viewState\.editing&&!stationState\.endedHandled/);
const crossfadeStart = player.data.raw.indexOf("async function startDualDeckCrossfade");
const selectionIndex = player.data.raw.indexOf('await api("/score/select","POST")', crossfadeStart);
const contextAdoptIndex = player.data.raw.indexOf("incoming.adoptContext(outgoing.ctx)", crossfadeStart);
const incomingPlayIndex = player.data.raw.indexOf("await incoming.play()", crossfadeStart);
