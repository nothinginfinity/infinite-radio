...
<body>
  ...
  <script>
    ...
    const editorState={session:null,previewMode:"draft"};
    ...
    async function previewEditorScore(){if(!viewState.editing||!editorState.session)return;const score=editorClone(editorState.previewMode==="original"?editorState.session.baseScore:editorState.session.draftScore),oldDuration=renderer.duration||score.durationSeconds,oldPosition=renderer.position,ratio=oldDuration?oldPosition/oldDuration:0;settleCrossfade();renderer.load(deepFreeze(editorValidate(score)));renderer.seek(clamp(ratio*renderer.duration,0,renderer.duration));try{await renderer.play();setAudioStatus();setStatus("Previewing "+(editorState.previewMode==="original"?"Original":"Draft")+" · local only");}catch(error){setStatus("Preview ready · tap Play: "+error.message);}renderEditor();}
