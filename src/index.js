...
<body>
  ...
  <script>
    ...
    async function ensureNextBuffered(session=stationState.session,options={}){const announce=options.announce!==false;if(!viewState.connected||session!==stationState.session)return false;if(stationState.queuedCount>0)return true;const active=stationState.prebufferPromise;if(active&&active.session===session)return active.promise;const pending=(async()=>{if(announce)setStatus("Preparing next composition…");try{const payload=await api("/score/prebuffer","POST",{listenerIntent:{surface:"v04_prebuffer",text:steeringPrompt()}});if(session!==stationState.session)return false;const queue=payload.state&&payload.state.compositionQueue||[];setStationBuffer(payload.composition_buffer_count,queue[0]||null);if(announce)setStatus(payload.created?(payload.fell_back?"Next score buffered via fixture fallback":"Next score buffered"):"Next score already buffered");return stationState.queuedCount>0;}catch(error){if(announce&&session===stationState.session)setStatus("Prebuffer retry available: "+error.message);return false;}})();stationState.prebufferPromise={session,promise:pending};try{return await pending;}finally{if(stationState.prebufferPromise&&stationState.prebufferPromise.promise===pending)stationState.prebufferPromise=null;}}
