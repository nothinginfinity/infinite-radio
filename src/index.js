...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/") {
      return new Response(
        `<!doctype html>
<html lang="en" data-step="v0.4-step-1">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <script>
    ...
    class ScoreRenderer{
      ...
      ensureMaster(){if(!this.ctx)throw new Error("audio_context_missing");if(!this.master){this.master=this.ctx.createGain();this.master.gain.value=.5;this.master.connect(this.ctx.destination);}return this.master;}
    ...
    const stationState={session:0,queuedCount:0,bufferedCompositionId:null,prebufferPromise:null,advanceInFlight:false,crossfadeInFlight:false,crossfadeOutgoing:null,autoAdvance:false,endedHandled:false};
    ...
    function stopAllDecks(){const outgoing=stationState.crossfadeOutgoing;if(outgoing&&outgoing!==renderer)outgoing.dispose();stationState.crossfadeOutgoing=null;stationState.crossfadeInFlight=false;renderer.stop();}
    ...
    async function returnToLive(){viewState.replaying=false;$("return-live").hidden=true;if(!viewState.connected)return;try{const payload=await api("/state");if(payload.state&&payload.state.currentComposition){loadScore(payload.state.currentComposition,{source:payload.state.currentComposition.provenance&&payload.state.currentComposition.provenance.composer||"persisted current"});setStatus("Back to the live station");}else{renderer.stop();canonicalState.score=null;$("score-title").textContent="No score loaded";setControls(false);setStatus("Back to the live station · no current composition yet");}}catch(error){setStatus(error.message);}}
    ...
    async function connect(){const session=++stationState.session;stopAllDecks();stationState.autoAdvance=false;stationState.endedHandled=false;viewState.connected=false;viewState.replaying=false;$("return-live").hidden=true;setStationBuffer(0);viewState.channelId=$("channel").value.trim();viewState.creatorId=$("creator").value.trim();if(!viewState.channelId||!viewState.creatorId){setStatus("Channel and creator are required");return;}setStatus("Opening channel…");try{await api("/init","POST",{creatorId:viewState.creatorId});const payload=await api("/state");if(session!==stationState.session)return;viewState.connected=true;$("next").disabled=false;$("steer-open").disabled=false;$("library-open").disabled=false;const queue=payload.state&&payload.state.compositionQueue||[];setStationBuffer(queue.length,queue[0]&&queue[0].compositionId);if(payload.state&&payload.state.currentComposition){loadScore(payload.state.currentComposition,{source:payload.state.currentComposition.provenance&&payload.state.currentComposition.provenance.composer||"persisted current"});void ensureNextBuffered(session);setStatus("Rejoined current composition · next score buffering");return;}if(stationState.queuedCount<1)await ensureNextBuffered(session);if(session!==stationState.session)return;if(stationState.queuedCount<1)throw new Error("composition_prebuffer_unavailable");await selectBufferedScore(session);setStatus("Station ready · next score buffering");}catch(error){if(session===stationState.session)setStatus(error.message);}}
    ...
    async function advanceAfterEnd(session){if(stationState.advanceInFlight||stationState.crossfadeInFlight||session!==stationState.session)return;stationState.advanceInFlight=true;setStatus("Advancing to buffered composition…");try{if(stationState.queuedCount<1)await ensureNextBuffered(session);if(session!==stationState.session)return;if(stationState.queuedCount<1)throw new Error("composition_prebuffer_unavailable");await selectBufferedScore(session,{autoplay:true});}catch(error){stationState.autoAdvance=false;if(session===stationState.session)setStatus("Station paused: "+error.message);}finally{stationState.advanceInFlight=false;}}
    async function startDualDeckCrossfade(session){if(stationState.crossfadeInFlight||stationState.advanceInFlight||session!==stationState.session||viewState.replaying||stationState.queuedCount<1)return false;stationState.crossfadeInFlight=true;const outgoing=renderer;stationState.crossfadeOutgoing=outgoing;let incoming=null;try{const payload=await api("/score/select","POST");if(session!==stationState.session)return false;const queue=payload.state&&payload.state.compositionQueue||[];setStationBuffer(payload.composition_buffer_count,queue[0]&&queue[0].compositionId);incoming=new ScoreRenderer();incoming.adoptContext(outgoing.ctx);incoming.rampMasterTo(0);loadScore(payload.score,{source:payload.score&&payload.score.provenance&&payload.score.provenance.composer||"persisted"},incoming);await incoming.play();outgoing.rampMasterTo(0,CROSSFADE_SECONDS);incoming.rampMasterTo(.5,CROSSFADE_SECONDS);renderer=incoming;stationState.autoAdvance=true;setStatus("Crossfading · next score buffering");void ensureNextBuffered(session);await new Promise(resolve=>setTimeout(resolve,CROSSFADE_SECONDS*1000));outgoing.dispose();if(stationState.crossfadeOutgoing===outgoing)stationState.crossfadeOutgoing=null;if(session===stationState.session)setStatus("Playing · transition complete · next score buffering");return true;}catch(error){if(incoming&&incoming!==renderer)incoming.dispose();stationState.autoAdvance=false;if(session===stationState.session)setStatus("Transition paused: "+error.message);return false;}finally{stationState.crossfadeInFlight=false;if(stationState.crossfadeOutgoing===outgoing&&outgoing!==renderer)stationState.crossfadeOutgoing=null;}}
    ...
    async function play(){try{await renderer.unlockFromGesture();setAudioStatus();await renderer.play();stationState.autoAdvance=true;stationState.endedHandled=false;setStatus("Playing · audio "+renderer.ctx.state+" · next score buffering");void ensureNextBuffered(stationState.session);}catch(error){setAudioStatus();setStatus(error.message);}}
    function tick(){const duration=renderer.duration||0,position=renderer.position,remaining=Math.max(0,duration-position);$("position").textContent=fmt(position);$("progress").style.width=(duration?clamp(position/duration*100,0,100):0)+"%";if(renderer.state==="playing"&&duration&&remaining<=Math.min(12,Math.max(4,duration*.25))&&stationState.queuedCount<1)void ensureNextBuffered(stationState.session);if(renderer.state==="playing"&&stationState.autoAdvance&&!viewState.replaying&&!stationState.crossfadeInFlight&&!stationState.advanceInFlight&&stationState.queuedCount>0&&remaining>0.15&&remaining<=CROSSFADE_LEAD_SECONDS)void startDualDeckCrossfade(stationState.session);if(renderer.state==="ended"&&stationState.autoAdvance&&!stationState.endedHandled&&!stationState.crossfadeInFlight){stationState.endedHandled=true;void advanceAfterEnd(stationState.session);}draw();requestAnimationFrame(tick);}requestAnimationFrame(tick);
    document.querySelectorAll("[data-view]").forEach(button=>button.addEventListener("click",()=>setProjection(button.dataset.view)));document.querySelectorAll("[data-steer]").forEach(input=>input.addEventListener("input",()=>{viewState.steering[input.dataset.steer]=Number(input.value);updateSteeringLabels();}));updateSteeringLabels();$("steer-open").addEventListener("click",()=>setSheet(true));$("steer-close").addEventListener("click",()=>setSheet(false));$("steer-cancel").addEventListener("click",()=>setSheet(false));$("sheet-backdrop").addEventListener("click",()=>{setSheet(false);setLibraryOpen(false);});$("steer-apply").addEventListener("click",applySteering);$("library-open").addEventListener("click",()=>setLibraryOpen(true));$("library-close").addEventListener("click",()=>setLibraryOpen(false));$("library-cancel").addEventListener("click",()=>setLibraryOpen(false));$("return-live").addEventListener("click",returnToLive);$("connect").addEventListener("click",connect);$("next").addEventListener("click",bufferNext);$("play").addEventListener("click",play);$("pause").addEventListener("click",()=>{renderer.pause();if(stationState.crossfadeOutgoing&&stationState.crossfadeOutgoing!==renderer)stationState.crossfadeOutgoing.pause();setAudioStatus();setStatus("Paused");});$("stop").addEventListener("click",()=>{stopAllDecks();stationState.autoAdvance=false;stationState.endedHandled=false;setAudioStatus();setStatus("Stopped");});window.addEventListener("infinite-radio-audio-state",setAudioStatus);document.addEventListener("visibilitychange",()=>{if(document.visibilityState!=="visible"||!renderer.ctx)return;setAudioStatus();if(renderer.state==="playing"&&renderer.ctx.state!=="running")setStatus("Tap Play to restore audio");});window.addEventListener("resize",draw);
  </script>
</body>
</html>`,
