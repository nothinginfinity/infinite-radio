...
<body>
  <div class="shell">
    ...
    <main>
      <section class="now"><div><div class="eyebrow">Now playing</div><div id="score-title" class="title">No score loaded</div><div id="score-meta" class="meta"><span>Validated infinite-radio-score-v1</span></div></div><div class="chips"><span id="schema-chip" class="chip">schema · waiting</span><span id="source-chip" class="chip">source · waiting</span><span id="provenance-chip" class="chip">composer · waiting</span><span id="audio-chip" class="chip warn">audio · tap play</span><span id="buffer-chip" class="chip">buffer · empty</span></div></section>
      <div class="modebar"><div class="tabs" role="tablist" aria-label="Station projection"><button class="tab active" data-view="visual">Visual</button><button class="tab" data-view="score">Score</button></div><button id="steer-open" class="steer" disabled>Steer next</button></div>
  ...
  <script>
    ...
    class ScoreRenderer{
      ...
      createContext(){if(!this.Context)throw new Error("webaudio_unavailable");this.audioSessionType=configurePlaybackAudioSession();this.ctx=new this.Context();this.master=this.ctx.createGain();this.master.gain.value=.5;this.master.connect(this.ctx.destination);this.ctx.onstatechange=()=>window.dispatchEvent(new CustomEvent("infinite-radio-audio-state",{detail:{state:this.ctx.state,sessionType:this.audioSessionType}}));return this.ctx;}
      async ensureContext(){if(!this.Context)throw new Error("webaudio_unavailable");this.audioSessionType=configurePlaybackAudioSession();if(!this.ctx||this.ctx.state==="closed")this.createContext();if(this.ctx.state!=="running"){try{await this.ctx.resume();}catch{}}if(this.ctx.state!=="running")throw new Error("audio_context_"+String(this.ctx.state||"unknown"));return this.ctx;}
    ...
    const viewState={channelId:null,creatorId:null,connected:false,activeView:"visual",sheetOpen:false,steering:{energy:60,tempo:55,brightness:55,density:50,space:55,tension:40}};
    const stationState={session:0,queuedCount:0,bufferedCompositionId:null,prebufferPromise:null,advanceInFlight:false,autoAdvance:false,endedHandled:false};
    ...
    function setSheet(open){viewState.sheetOpen=Boolean(open);$("steer-sheet").classList.toggle("open",viewState.sheetOpen);$("sheet-backdrop").classList.toggle("open",viewState.sheetOpen);}
