...
<body>
  ...
  <script>
    ...
    async function play(){try{await renderer.unlockFromGesture();setAudioStatus();await renderer.play();const livePlayback=!viewState.replaying;stationState.autoAdvance=livePlayback;stationState.endedHandled=false;setStatus(livePlayback?"Playing · audio "+renderer.ctx.state+" · next score buffering":"Playing library replay · live continuity unchanged");if(livePlayback)void ensureNextBuffered(stationState.session);}catch(error){setAudioStatus();setStatus(error.message);}}
