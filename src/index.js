...
<body>
  ...
  <script>
    ...
    async function rejoinLiveState(session=stationState.session,{resumeIfPlaying=false}={}){const wasPlaying=renderer.state==="playing";const payload=await api("/playback/rejoin","POST",{});if(session!==stationState.session)return null;const queue=payload.state&&payload.state.compositionQueue||[];setStationBuffer(queue.length,queue[0]&&queue[0].compositionId);const score=payload.state&&payload.state.currentComposition;if(score){const currentId=canonicalState.score&&canonicalState.score.compositionId;if(currentId!==score.compositionId)loadScore(score,{source:score.provenance&&score.provenance.composer||"persisted current"});applyPlaybackSnapshot(payload.playback);if(resumeIfPlaying&&wasPlaying&&!payload.playback.ended)await renderer.play();}return payload;}
